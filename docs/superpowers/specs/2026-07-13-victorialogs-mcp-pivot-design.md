...
- **丟**:自建 DuckLake 儲存層(`internal/store`、metrics、checkpoint、diskguard、ingest handler、health、server config、cmd/ducklog)—— 沉沒成本,但那是水管。
...
- **移除**:`internal/*`(store, metrics, auth, ratelimit, diskguard, checkpoint, ingest, health, config)、`cmd/ducklog`。plan 會有一個獨立 task 乾淨移除,git 歷史保留(需要時 revert 得回來)。
