...
- **模組**:MCP server 在 root `docklog` 模組(移除 go-duckdb 後為純 Go);`client/` transport 為既有獨立模組。
...
```
go.mod / go.sum              root 模組 docklog:移除 go-duckdb,加入 mcp-go
cmd/docklog-mcp/main.go      MCP server 入口(stdio)
...
cmd/docklog-mcp/main_test.go MCP 協定 smoke(stdio handshake + 呼叫一個 tool)
...
internal/config, cmd/docklog   (Task 8)
```
...
**Files:**
- Modify: `client/e2e_test.go`(改成打真 VL,而非已移除的 docklog server)
...
Expected: FAIL(舊測試 build `../cmd/docklog` 已不適用 / 尚未改寫)。
...
git -c user.name='docklog' -c user.email='dev@docklog.local' commit -m "feat(client): repoint transport e2e to VictoriaLogs ingest"
```
...
- [ ] **Step 3: 執行確認失敗** — Run: `cd /home/dva/workspace/docklog && GOTOOLCHAIN=go1.24.0 go test ./internal/vl/`(此時 root 仍有 go-duckdb,故仍需 GOTOOLCHAIN);Expected: FAIL(`New` 未定義)。
...
git -c user.name='docklog' -c user.email='dev@docklog.local' commit -m "feat(vl): LogsQL HTTP client + query builders"
```
...
git -c user.name='docklog' -c user.email='dev@docklog.local' commit -m "feat(bound): anti-confabulation contract (token-bound, explicit downgrade)"
```
...
git -c user.name='docklog' -c user.email='dev@docklog.local' commit -m "feat(tools): summarize_errors, get_trace, search_logs, compare_periods over LogsQL"
```
...
**Files:**
- Create: `internal/mcpserver/server.go`, `cmd/docklog-mcp/main.go`, `cmd/docklog-mcp/main_test.go`, `config.example.yaml`
...
```
寫一個最小 spike(可丟在 `cmd/docklog-mcp/spike_test.go` 或直接讀 mcp-go 的 `server` package 文件/godoc)確認實際 API:server 建立、`AddTool`/`RegisterTool` 名稱、tool 定義(`mcp.NewTool` + `mcp.WithString(...)`)、handler 簽名(`func(ctx, mcp.CallToolRequest) (*mcp.CallToolResult, error)`)、參數取值(`req.Params.Arguments` / helper)、回傳文字結果(`mcp.NewToolResultText(jsonString)`)、`server.ServeStdio(s)`。**以實際安裝版本為準**,下面 code 若與 API 不符,依實際調整(這是唯一未在本專案驗證過的相依)。

- [ ] **Step 2: 寫 smoke 測試**

`cmd/docklog-mcp/main_test.go`:對 `internal/mcpserver` 做「不需真 VL」的最小驗證 —— 用一個假的 tool 回應路徑或直接測 tool 註冊數/名稱。若 mcp-go 難以在測試中驅動 stdio,改為:測 `internal/mcpserver` 的 tool schema 組裝(名稱、參數)正確,並在 report 說明 stdio handshake 用手動 `docklog-mcp` + `mcp` client 驗證。
...
func Serve(ctx context.Context, c *vl.Client) error {
	s := server.NewMCPServer("docklog", "0.1.0")
...
- [ ] **Step 4: 執行 + build** — Run: `GOTOOLCHAIN=go1.24.0 go build ./cmd/docklog-mcp && go test ./internal/mcpserver/ ./cmd/docklog-mcp/`;Expected: build OK,測試 PASS。

- [ ] **Step 5: Commit**

```bash
git add internal/mcpserver/ cmd/docklog-mcp/ config.example.yaml go.mod go.sum
git -c user.name='docklog' -c user.email='dev@docklog.local' commit -m "feat(mcp): stdio MCP server exposing 4 LogsQL-backed tools"
```
...
**Files:**
- Delete: `internal/store`, `internal/metrics`, `internal/auth`, `internal/ratelimit`, `internal/diskguard`, `internal/checkpoint`, `internal/ingest`, `internal/health`, `internal/config`, `cmd/docklog`
- Modify: `go.mod`/`go.sum`(`go mod tidy` 移除 go-duckdb 及其相依)

**Interfaces:** 無新增。目標:root 模組只剩 MCP server(vl/bound/tools/mcpserver + cmd/docklog-mcp),不再依賴 go-duckdb。
...
```bash
git add -A
git -c user.name='docklog' -c user.email='dev@docklog.local' commit -m "refactor: remove superseded DuckLake storage layer (superseded by VictoriaLogs)"
```
...
`scripts/run-dev.sh`:啟 VictoriaLogs(找 binary from `VL_BINARY` 或提示下載)於 `127.0.0.1:9428`、印出 vmui URL、然後 `exec ./docklog-mcp`(stdio,供 Claude Code 掛)。或分兩步:`run-dev.sh vl`(起 VL)、`run-dev.sh mcp`(起 MCP)。

- [ ] **Step 2: README**

`README.md`:轉向說明(VL backend + MCP 工具層)、如何跑(下載 VL OSS binary、起 VL、build `docklog-mcp`、在 Claude Code 註冊此 MCP server 的 stdio 指令)、4 個 tool 的用途、transport 如何指向 VL、已知限制(v1 無 auth、2 process)。連結設計 spec。

- [ ] **Step 3: Commit**

```bash
git add scripts/ README.md
git -c user.name='docklog' -c user.email='dev@docklog.local' commit -m "docs: run scripts + README for VictoriaLogs + MCP setup"
```
